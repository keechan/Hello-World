		...
		<div align=center>
			...
			<table border=1 width=500>
				...
				<% for( MemberVO m : li ) { 
					...
					//String REGIST_MONTH = df.format(m.getCLASS_PRICE());
				...
				%>
				<tr>
					<td align=center><%=m.getREGIST_MONTH() %></td>
					<td align=center><%=m.getC_NO() %></td>
					<td align=center><%=m.getC_NAME() %></td>
					<td align=right><%=m.getCLASS_NAME() %></td>
					<td align=center><%=m.getCLASS_AREA() %></td>
					<td align=center><%=m.getTUITION() %></td>
					<td align=center><%=m.getGRADE() %>
				</tr>
				<% } %>
			</table>			
		</div> 
